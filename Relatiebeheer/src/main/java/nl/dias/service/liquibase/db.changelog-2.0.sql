...
--changeset Patrick Heidotting:123
ALTER TABLE `KANTOOR` ADD COLUMN `IPADRES` varchar(30) NULL;

--changeset Patrick Heidotting:124
ALTER TABLE `KANTOOR` DROP COLUMN `BTW_NUMMER`;
ALTER TABLE `KANTOOR` DROP COLUMN `DATUMOPRICHTING`;
ALTER TABLE `KANTOOR` DROP COLUMN `SOORTKANTOOR`;

--changeset Patrick Heidotting:125
CREATE TABLE `MAILCHECK` (
  `ID` bigint(20) NOT NULL AUTO_INCREMENT,
  `GEBRUIKER` bigint(20) NOT NULL,
  `MAILADRES` varchar(50) NOT NULL,
  PRIMARY KEY (`ID`)
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 AUTO_INCREMENT=1 ;

--changeset Patrick Heidotting:126
ALTER TABLE `GEBRUIKER` DROP COLUMN `OAUTHTODOIST`;
ALTER TABLE `GEBRUIKER_AUD` DROP COLUMN `OAUTHTODOIST`;

--changeset Patrick Heidotting:127
ALTER TABLE `SCHADE` CHANGE COLUMN `DATUMTIJDMELDING` `DATUMMELDING` date;
ALTER TABLE `SCHADE` CHANGE COLUMN `DATUMTIJD` `DATUM` date;
ALTER TABLE `SCHADE_AUD` CHANGE COLUMN `DATUMTIJDMELDING` `DATUMMELDING` date;
ALTER TABLE `SCHADE_AUD` CHANGE COLUMN `DATUMTIJD` `DATUM` date;

--changeset Patrick Heidotting:128
ALTER TABLE `GEBRUIKER` DROP COLUMN `BSN`;
ALTER TABLE `GEBRUIKER_AUD` DROP COLUMN `BSN`;

--changeset Patrick Heidotting:129
DROP TABLE IF EXISTS `MAILCHECK`;
DROP TABLE IF EXISTS `POLIS`;
DROP TABLE IF EXISTS `POLIS_AUD`;
DROP TABLE IF EXISTS `SCHADE`;
DROP TABLE IF EXISTS `SCHADE_AUD`;
DROP TABLE IF EXISTS `SOORTSCHADE`;
DROP TABLE IF EXISTS `STATUSSCHADE`;
DROP TABLE IF EXISTS `STATUSSCHADE_AUD`;
DROP TABLE IF EXISTS `VERZEKERINGSMAATSCHAPPIJ`;

--changeset Patrick Heidotting:130
