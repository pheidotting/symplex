...
define puppi::project::svn (
  ...
  puppi::deploy { "${name}-Deploy_Files":
    priority  => '40',
    command   => 'svn.sh',
    arguments => "-a deploy -s ${source} -d ${deploy_root} -u ${user} -gs ${svn_subdir} -su ${svn_user} -sp ${
      svn_password} -t ${tag} -b ${branch} -c ${commit} -v ${bool_verbose} -k ${bool_keep_svndata} -e ${bool_svn_export}
      ",
    user      => 'root',
    project   => $name,
    enable    => $enable,
  ...
  if ($bool_backup_enable == true) {
    ...
    if ($bool_keep_svndata != true) {
      puppi::rollback { "${name}-Rollback_Files":
        priority  => '40',
        command   => 'svn.sh',
        arguments => "-a rollback -s ${source} -d ${deploy_root} -gs ${svn_subdir} -t ${tag} -b ${branch} -c ${commit}
           -v ${bool_verbose} -k ${bool_keep_svndata}",
        user      => $user,
        project   => $name,
        enable    => $enable,
      }
