...
define puppi::project::y4maven (
  ...
  $platform_metadata_arguments = $http_password ? {
    ...
    default => "-s ${source}/hybrisServer-Platform/maven-metadata.xml -t maven-metadata -a ${real_always_deploy} -u ${
      http_user} -p ${http_password}"
  }
  ...
  puppi::deploy { "${name}-Extract_Maven_Metadata_hybrisServer-Platform":
    ...
    arguments => "-m ${document_suffix} -mc ${config_suffix} -mj ${jar_suffix} -mw ${war_suffix} -mz ${zip_suffix} -at $
      {artifact_type}",
    user      => 'root',
    project   => $name,
    enable    => $enable,
  ...
  $extensions_metadata_arguments = $http_password ? {
    ...
    default => "-s ${source}/hybrisServer-AllExtensions/maven-metadata.xml -t maven-metadata -a ${real_always_deploy}
       -u ${http_user} -p ${http_password}",
  }
  ...
  if ($config_root != '') {
    ...
    $config_metadata_arguments = $http_password ? {
      ...
      default => "-s ${source}/config/maven-metadata.xml -t maven-metadata -a ${real_always_deploy} -u ${http_user} -p $
        {http_password}"
    }
  ...
  if ($config_root != '') {
    puppi::deploy { "${name}-Backup_Existing_ConfigDir":
      priority  => '30',
      command   => 'archive.sh',
      arguments => "-b ${config_root} -t config -d predeploydir_configfile -o '${backup_rsync_options}' -n ${
        backup_retention}",
      user      => 'root',
      project   => $name,
      enable    => $enable,
  ...
  if ($document_root != '') {
    puppi::deploy { "${name}-Backup_Existing_DocumentDir":
      priority  => '30',
      command   => 'archive.sh',
      arguments => "-b ${document_root} -t docroot -d predeploydir_configfile -o '${backup_rsync_options}' -n ${
        backup_retention}",
      user      => 'root',
      project   => $name,
      enable    => $enable,
