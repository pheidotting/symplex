...
define puppi::project::hg (
  ...
  puppi::deploy { "${name}-Deploy_Files":
    priority  => '40',
    command   => 'hg.sh',
    arguments => "-a deploy -s ${source} -d ${deploy_root} -u ${user} -t ${tag} -b ${branch} -c ${commit} -v ${
      bool_verbose} -k ${bool_keep_hgdata}",
    user      => 'root',
    project   => $name,
    enable    => $enable,
  }
  ...
  if ($bool_keep_hgdata != true) {
    puppi::rollback { "${name}-Rollback_Files":
      priority  => '40',
      command   => 'hg.sh',
      arguments => "-a rollback -s ${source} -d ${deploy_root} -t ${tag} -b ${branch} -c ${commit} -v ${bool_verbose}
         -k ${bool_keep_hgdata}",
      user      => $user,
      project   => $name,
      enable    => $enable,
