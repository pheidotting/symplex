...
define puppi::project::git (
  ...
  puppi::deploy { "${name}-Deploy_Files":
    priority  => '40',
    command   => 'git.sh',
    arguments => "-a deploy -s ${source} -d ${deploy_root} -u ${user} -gs ${git_subdir} -t ${tag} -b ${branch} -c ${
      commit} -v ${bool_verbose} -k ${bool_keep_gitdata}",
    user      => 'root',
    project   => $name,
    enable    => $enable,
  }
  ...
  if ($bool_keep_gitdata != true) {
    puppi::rollback { "${name}-Rollback_Files":
      priority  => '40',
      command   => 'git.sh',
      arguments => "-a rollback -s ${source} -d ${deploy_root} -gs ${git_subdir} -t ${tag} -b ${branch} -c ${commit} -v
        ${bool_verbose} -k ${bool_keep_gitdata}",
      user      => $user,
      project   => $name,
      enable    => $enable,
    }
