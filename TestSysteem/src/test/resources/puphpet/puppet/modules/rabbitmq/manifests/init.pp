...
class rabbitmq (
  ...
  validate_re($cluster_node_type, '^(ram|disc|disk)$')
  # Both disc and disk are valid http://www.rabbitmq.com/clustering.html
  ...
  if $package_source { # $package_source was specified by user so use that one
    ...
    case $::osfamily {
      'RedHat', 'SUSE': {
        ...
        $real_package_source = "http://www.rabbitmq.com/releases/rabbitmq-server/v${base_version}/rabbitmq-server-${
          version}.noarch.rpm"
      }
      'OpenBSD': {
