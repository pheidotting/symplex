...
define puphpet::alternatives_update (
  ...
  if !$optional {
    ...
    exec { "check alternatives for ${item}":
      ...
      command => "echo Alternative for ${item} version containing ${versiongrep}
         was not found, or multiple found ; false",
  ...
  exec { "update alternatives for ${item} to ${versiongrep}":
    path    => ['/sbin', '/bin', '/usr/bin', '/usr/sbin'],
    command => "${altcmd} --set ${item} $( ${altcmd} --display ${item} | grep '^/' | grep ${versiongrep}
       | sed 's/ .*$//')",
