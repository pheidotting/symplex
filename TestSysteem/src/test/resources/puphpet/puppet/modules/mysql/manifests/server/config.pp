...
class mysql::server::config {
  ...
  if $mysql::server::manage_config_file {
    ...
    if $configparentdir != '/' and $configparentdir != '/etc' and $configparentdir != $includedir and $configparentdir
      != mysql_dirname($includedir) {
  ...
  if $options['mysqld']['ssl-disable'] {
    notify { 'ssl-disable':
      message =>
        'Disabling SSL is evil! You should never ever do this except if you are forced to use a mysql version compiled without SSL support'
    }
  }
}
