...
class composer (
  ...
  case $download_method {
    'curl': {
      $download_command = "curl -sS https://getcomposer.org/installer | ${composer::php_bin} -- --install-dir=${tmp_path
        } --filename=${composer_file}"
  ...
  exec { "move_composer_${target_dir}":
    command     => "mv ${tmp_path}/${composer_file} ${target_dir}/${composer_file}; chmod 0755 ${target_dir}/${
      composer_file}",
    refreshonly => true,
  }
